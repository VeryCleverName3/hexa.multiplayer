...
ws.onmessage = (e)=>{
  tempHexagons = JSON.parse(e.data).hexagons;
  diff = [];
  for(var i = 0; i < tempHexagons.length; i++){
    if(hexagons[i][2] != tempHexagons[i][2]) diff.push(i);
  }
  for(i of diff){
    hexagons[i][2] = tempHexagons[i][2];
    hexagons[i][4] = tempHexagons[i][4];
    flip(i, hexagons[i][2]);
  }
}
...
function  update(mX, mY){
  ...
  ws.send(JSON.stringify({hexagons: hexagons}));
}
